...
### 5. 新功能和需求演进必须维护 OpenSpec Wiki

为了让未来的维护者和 AI 能快速理解系统，功能知识必须沉淀到 `openspec/wiki/`。

- 每次完成 `feature` 后，必须在 `openspec/wiki/` 新建或更新对应主题文档
- 文档至少要说明：新增或更新了什么功能、涉及哪些关键文件、当前行为边界或注意事项
- 每次完成 `bugfix`、`behavior-change` 或需求调整后，必须同步修改对应 wiki 文件，避免 wiki 与真实实现漂移
- wiki 文档应优先按功能主题长期维护，而不是只写一次性的流水账
...
### 7. 非琐碎 AI 编码任务默认使用 Generator + Evaluator
...
- `openspec/wiki/` 用于沉淀面向维护者的长期功能说明，不参与运行时检索。
...
2. 已更新相关 OpenSpec 文档
3. 已创建或更新对应的 `openspec/wiki/` 文档
4. 已按正确层级完成验证
5. 最终交付能清楚说明改了什么、改在哪里、如何验证

## 后续事项
- 每次实现新功能或完成重要变更后，务必更新 `README.md`，确保文档准确反映当前项目状态和使用方式。
- 每次实现新功能、修复重要 BUG 或调整需求后，务必同步维护 `openspec/wiki/` 中对应的主题文档。
