...
- 日期：2026-04-01
- 类型：`behavior-change`
- 摘要：引入通用 Agent Engine，并让 chat workflow 退化为可组合配置

## 本次更新内容

- 引入 `src/lib/agent.ts` 作为通用 `Agent` 引擎，统一 `plan -> execute -> evaluate` 主循环
- 新增 `src/server/chat/chat-agent.ts`，把 chat 的 planner / executor / evaluator / memory 以依赖注入方式组装起来
- 将 `chat-orchestrator` 收缩为轻量入口，不再自己维护写死的 while-loop
- 保留现有流式事件、执行日志、记忆更新和 Prompt Layer 能力

## 当前系统能力

- 支持基于 Next.js App Router 的聊天页面和流式消息返回
- 支持推理过程展示和 Token 消耗观察
- 支持可复用 Agent Engine 抽象
- 支持集中式 Prompt Layer 与模板化消息构建
...
- `src/server/chat/chat-agent.ts`：Chat 场景的 Agent 装配
...
- `src/lib/agent.ts`：通用 Agent 引擎与 loop 抽象
- `src/lib/memory.ts`：短期记忆和摘要记忆
- `src/lib/long-term-memory.ts`：长期记忆模型
- `src/lib/prompt-builder.ts`：集中式 Prompt Layer、模板注册与变量注入
