import type { UserProfile } from '@/src/lib/long-term-memory'
...
async function persistLongTermMemory(userMessage: string, assistantMessage: string) {
...
async function* generateStreamResponse(userMessage: string) {
  ...
  for (let round = 0; round < MAX_TOOL_ROUNDS; round += 1) {
    ...
    if (completion.toolCalls.length === 0) {
      ...
      await persistLongTermMemory(userMessage, finalContent)
      return
    }
